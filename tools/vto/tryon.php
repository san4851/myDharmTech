 ...
 * Saves uploads, builds a category prompt with public image URLs,
 * and sends both photos as image inputs to OpenAI gpt-image-1 edits.
 */
...
function vto_fail(int $status, string $message, array $errors = []): void
{
...
function vto_save_upload(array $file, string $role, string $uploadDir): array
{
    if (($file['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_OK) {
        vto_fail(400, ucfirst($role) . ' image upload failed.', ['code' => $file['error'] ?? 0]);
    }
...
function vto_openai_edit(string $apiKey, string $model, string $prompt, string $size, array $userFile, array $itemFile): array
{
    ...
    if ($ch === false) {
        vto_fail(500, 'Could not start OpenAI request.');
    }
    ...
    if ($raw === false) {
        vto_fail(502, 'OpenAI request failed: ' . $curlError);
    }

    $decoded = json_decode((string) $raw, true);
    if (!is_array($decoded)) {
        vto_fail(502, 'OpenAI returned an invalid response.');
    }
    if ($http >= 400) {
        $msg = (string) ($decoded['error']['message'] ?? 'OpenAI error.');
        vto_fail(502, $msg, ['openai_status' => $http]);
    }
    ...
    return ['b64' => is_string($b64) ? $b64 : null, 'url' => is_string($url) ? $url : null];
}
...
if ($apiKey === '') {
    vto_fail(500, 'OPENAI_API_KEY is not configured in .env.');
}
